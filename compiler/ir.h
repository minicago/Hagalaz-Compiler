...
public:
    explicit Module() {
        void_ty_ = std::make_shared<SimpleType>(VOID);
        label_ty_ = std::make_shared<SimpleType>(VOID);
        int32_ty_ = std::make_shared<SimpleType>(INT);
        float32_ty_ = std::make_shared<SimpleType>(FLOAT);
    }
    ~Module() = default;

    virtual std::string print();
    void add_global_variable(std::shared_ptr<GlobalVariable> g) { global_list_.push_back(g); }
    void add_function(std::shared_ptr<Function> f) { function_list_.push_back(f); }
    ...
    std::shared_ptr<ArrayType> get_array_type(std::shared_ptr<Type> contained, unsigned num_elements) {
        if (!array_map_.count({contained, num_elements})) {
            array_map_[{contained, num_elements}] = std::make_shared<ArrayType>(contained, num_elements);
        }
        return array_map_[{contained, num_elements}];
    }

    std::shared_ptr<Function> getMainFunc();

    std::vector<std::shared_ptr<GlobalVariable>> global_list_;
    std::vector<std::shared_ptr<Function>> function_list_;

    std::shared_ptr<SysyType> int32_ty_;
    std::shared_ptr<SysyType> float32_ty_;
    std::shared_ptr<SysyType> label_ty_;
    std::shared_ptr<SysyType> void_ty_;
    std::map<std::shared_ptr<Type>, std::shared_ptr<PointerType>> pointer_map_;
    std::map<std::pair<std::shared_ptr<Type>, int>, std::shared_ptr<ArrayType>> array_map_;
};
